...
| Orchestration Saga | MassTransit State Machine | Multi-step workflows with compensation | Booking: reserve → pay → confirm / compensate |
...
The booking workflow spans Travel, Payment, and downstream services. An **orchestration-based Saga** (MassTransit State Machine) in the Travel Booking Service coordinates the multi-step process. Every state transition persists new saga state **and** outgoing commands/events into the Outbox within a single transaction — a step can never "half-complete."

```mermaid
flowchart LR
    subgraph TravelBookingService["Travel Booking Service"]
        Saga["Saga Orchestrator<br/>Requested → InventoryReserved<br/> → PaymentProcessing<br/> → Confirmed | Compensating → Failed"]
    ...
    TDB -.->|"2. Poll unsent"| Disp
    Disp -->|"3. Publish"| SB
    SB -->|"4. ProcessPayment cmd"| Pay
    Pay -->|"5. PaymentSucceeded"| SB
    SB -->|"6. Correlate to Saga"| Saga
    Saga -->|"7. BookingConfirmed<br/>→ Outbox"| TDB
    Disp -->|"8. Publish"| SB
    SB -->|"9. Fan-out"| N & R & E
```
...
|:---|:---|:---|
| Inventory reservation fails | None — nothing committed | Saga → `Failed` |
...
```mermaid
flowchart TD
    P["Publisher"] -->|"Transactional Outbox"| SB["Service Bus<br/>(MaxDelivery:5 · TTL:24h)"]
    SB --> C["Consumer<br/>Polly: 3 retries · 2s→4s→8s<br/>+ Inbox idempotency check"]
    C -->|OK| Done["Complete()"]
    C -->|Fail| Ret["Abandon → Redeliver"]
    Ret -->|"< MaxDelivery"| SB
    Ret -->|"≥ MaxDelivery"| DLQ["Dead-Letter Queue"]
    DLQ --> AF["Azure Function<br/>Classify · Alert · Park"]
```
...
| **Saga consistency** | State + commands persisted atomically; compensation on every failure path |
...
| 2 | **Saga stuck** — Payment ACL unresponsive | M | H | 30s saga timeout (scheduled message); compensation releases inventory; Polly CB on ACL | Saga instances in `PaymentProcessing` > 5 min, CB state |
...
| | Detail |
|:---|:---|
| **Tools** | Gemini 3.1 Pro, GitHub Copilot, Claude Opus 4.6 |
